...
# =============================================================================
# Aggregate methods on Column
# =============================================================================


class TestAggregates:
    def test_count(self):
        """Column.count() produces COUNT(col)."""
        expr = User.id.count()
        params: list[Any] = []
    ...
    def test_aggregate_with_alias(self):
        ...
        expr = User.id.count().as_("user_count")
        params: list[Any] = []
        result = expr.to_sql(params)
    ...
    def test_sum_with_alias_in_select(self):
        ...
        query = (
            SelectQuery[Any](None, (User.role, User.age.sum().as_("total_age")))
            .from_(User)
            .group_by(User.role)
        )
        s, params = query.build()
...
class TestHaving:
    def test_having_basic(self):
        ...
        query = (
            SelectQuery[Any](None, (User.role, User.id.count()))
            .from_(User)
            .group_by(User.role)
            .having(User.id.count() > 5)
        )
        s, params = query.build()
        assert "GROUP BY users.role" in s
    ...
    def test_having_multiple(self):
        ...
        query = (
            SelectQuery[Any](None, (User.role,))
            .from_(User)
            .group_by(User.role)
            .having(User.id.count() > 2)
            .having(User.age.avg() < 50)
        )
        s, params = query.build()
        assert "HAVING" in s
        assert "AND" in s
...
class TestCase:
    def test_simple_case(self):
        """Column.case() for simple CASE with mapping."""
        expr = User.role.case({"admin": 1, "user": 0}, else_=-1)
        params: list[Any] = []
        result = expr.to_sql(params)
    ...
    def test_simple_case_no_else(self):
        """CASE without ELSE."""
        expr = User.role.case({"admin": 1, "user": 0})
        params: list[Any] = []
        result = expr.to_sql(params)
        assert "CASE users.role" in result
    ...
    def test_case_with_alias(self):
        """CASE with .as_() alias."""
        expr = User.role.case({"admin": 1}, else_=0).as_("role_num")
        params: list[Any] = []
        result = expr.to_sql(params)
    ...
    def test_case_in_select(self):
        ...
        query = SelectQuery[Any](
            ...
            (
                User.name,
                User.role.case({"admin": "Admin"}, else_="Member"),
            ),
        ).from_(User)
        s, params = query.build()
    ...
    def test_case_comparison_operators(self):
        ...
        expr = User.role.case({"admin": 1}, else_=0) > 0
        params: list[Any] = []
        result = expr.to_sql(params)
        assert "CASE users.role" in result
        assert "> $" in result
